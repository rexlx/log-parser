...
var (
	...
	remote = flag.Bool("remote", false, "connect to remote host")
	addr   = flag.String("addr", "", "remote address")
	port   = flag.Int("port", 22, "remote ssh port")
	uname  = flag.String("user", "", "username")
)
...
func main() {
	...
	case *remote:
		data := make([]byte, 1024)
		cli := NewSshClient(*addr, *uname, *port)
		...
		err = sess.Run("journalctl -f -o json")
		if err != nil {
			log.Fatalln(err)
		}
